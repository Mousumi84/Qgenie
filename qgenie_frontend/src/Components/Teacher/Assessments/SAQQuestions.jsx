...
const SAQQuestions = ({ item, index }) => {
    console.log("SAQ", item);
    let question = [];
    ...
    for (let i = 0; i < item.questionCount; i++) {
        question.push(
            <div key={`${item.type}-question-${i + 1}`} className="border border-gray-300 bg-gray-50 p-4 rounded-md relative">
                {/* Question Type */}
                <Form.Item name={["questions", i, "questionType"]} initialValue="SAQ" hidden>
                    <Input />
                </Form.Item>
                ...
                <Form.Item name={["questions", i, "sampleAnswer"]} label="Answer">
                    <Input placeholder="Enter option answer" />
                </Form.Item>
        ...
        <>
            <Collapse
                ...
                items={[
                    {
                        key: "1",
                        label: (<h3 className="font-semibold text-lg" name={["questions", index, "question"]}>Short Answer Question</h3>),
