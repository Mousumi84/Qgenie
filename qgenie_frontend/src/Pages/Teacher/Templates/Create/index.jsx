...
function TeacherTemplatesCreate() {
  ...
  const Content = () => {
    return (
      <div
        className="p-2 w-full border border-gray-300 rounded-md flex flex-col gap-2 cursor-pointer"
        name="questionType"
      >
        {quesTypeOpt.map((option) => (
          <div
            ...
            onClick={() => addQuestionType(option)}
          >
            {option.label}
          </div>
        ))}
  ...
  useEffect(() => {
    dispatch(
      headingUpdate({
        ...
        subheading: "This will help you create multiple templates",
      }),
    );
  }, [dispatch]);
  ...
  return (
    <div id="TeacherTemplatesCreate">
      <div className="border border-green-100 rounded-lg p-4">
        <Form labelCol={{ span: 5 }} labelAlign="left" wrapperCol={{ span: 20 }} layout="horizontal" className="w-11/12 flex flex-col gap-2" onFinish={onSubmit}>
          ...
          <div className="flex flex-col gap-2">
            {open && <Content />}
            <Button onClick={() => setOpen(!open)} type="primary" ghost>
              Question Types
            </Button>
          </div>
          ...
          <div className="flex flex-col gap-4">
            {questionTypes.map((item, index) => {
              ...
              return (
                <div key={index}>
                  <Form.Item
                    name={["questionTypeTemplate", index, "type"]}
                    hidden
